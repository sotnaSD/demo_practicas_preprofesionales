...
def exportarVideos(request):
    ...
    export.append(["Fecha de Consulta","VideoId","Titulo Video","Fecha Publicación","Numero Vistas","Numero Likes","Numero Dislikes","Numero Comentarios"])
    ...
    # acceder a campos con relaciones y no solo al id
        export.append([
            result.fechaConsulta, result.videoId, result.titulo, result.fechaPublicacion, result.numeroVistas, result.numeroLikes,
    ...
    return excel.make_response(sheet, "csv", file_name= 'youtube_video_data_analysis'+(datetime.datetime.now().strftime("%Y-%m-%d-%H-%M"))+'.csv')

def exportarComentarios(request):
    export = []
    # Se agregan los encabezados de las columnas
    export.append(["Fecha de Consulta","VideoId","Fecha Publicacion","Comentario","Trend","Numero Likes","Comentario Respuestas","Es Toplevel"])
    # Se obtienen los datos de la tabla o model y se agregan al array
    ...
    # acceder a campos con relaciones y no solo al id
        export.append([
            result.fechaConsulta, result.videoId, result.fechaPublicacion, result.comentario, result.trend, result.comentarioLikes,
    ...
    # se devuelve como "Response" el archivo para que se pueda "guardar"
    # en el navegador, es decir como hacer un "Download"
    return excel.make_response(sheet, "csv", file_name= 'youtube_comments_data'+(datetime.datetime.now().strftime("%Y-%m-%d-%H-%M"))+'.csv')
